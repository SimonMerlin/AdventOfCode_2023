...
class Grid:
    ...
    def countEnergizedCellsAndReset(self):
        value = len(set([(e[0], e[1]) for e in self.energizedCells]))
        self.energizedCells = []
        return value
...
def main():
    ...
    for y in range(len(lines)):
        checkBeam(0, y, 'r', grid)
        gridValue = grid.countEnergizedCellsAndReset()
        if gridValue > maxValue:
            maxValue = gridValue
        checkBeam(len(lines[0])-1, y, 'l', grid)
        gridValue = grid.countEnergizedCellsAndReset()
        if gridValue > maxValue:
            maxValue = gridValue
    for x in range(len(lines[0])):
        checkBeam(x, 0, 'd', grid)
        gridValue = grid.countEnergizedCellsAndReset()
        if gridValue > maxValue:
            maxValue = gridValue
        checkBeam(x, len(lines)-1, 'u', grid)
        gridValue = grid.countEnergizedCellsAndReset()
        if gridValue > maxValue:
            maxValue = gridValue
    return maxValue 
